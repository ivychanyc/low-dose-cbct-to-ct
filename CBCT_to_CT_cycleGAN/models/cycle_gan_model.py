...
class CycleGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.netE_B.load_state_dict(torch.load('/data/Unpaired_MR_to_CT_Image_Synthesis-master/checkpoints/CP5.pth'))
        ...
        if self.isTrain:
            ...
            #print('I am here 3', self.gpu_ids)
        ...
        if self.isTrain:
            ...
            #self.device=torch.device("cuda:0") #Ivy commented this on 03.08.2021
    ...
    def set_input(self, input):
        ...
        self.mask_A_mid= input['M_A']
        self.image_paths = input['A_paths' if AtoB else 'B_paths']
        self.image_pathsA = input['A_paths']
        self.image_pathsB = input['B_paths']
        # self.maskA_path = input['A_paths'][0].replace('trainA','maskA').replace('mr','mrmask') #commented on 07.10.2021
    ...
    def forward(self):
        ...
        self.fake_B_adj = (self.fake_B - (-1)) / (1 - (-1)) * 1.0 # Ivy commented this on 06.08.2021
        #self.fake_B_adj = (self.fake_B - (-1)) / (1 - (-1)) * 65535.0 
        ...
        #a.set_title('Input real_A image')
        #plt.imshow(self.real_A.cpu().detach().numpy()[0][0], cmap="gray")
        ##plt.colorbar()
        #plt.savefig('my_plot_real_A_A.png') 
        ...
        # self.mask_A_mid = io.imread(self.maskA_path) # commneted 07.10.2021
        ...
        #     print(self.data['A_paths'][0])
        # print()
        self.mask_A = self.mask_A_mid.cuda()
        #print('***************************halohalo',self.mask_A .size())
        #print('***************************halohalo',self.real_A .size())

        # self.mask_A = torch.from_numpy(transform.resize(self.mask_A_mid, (256, 256))).float().cuda().unsqueeze(0).unsqueeze(0)
        # self.mask_A = torch.from_numpy(transform.resize(self.mask_A_mid, (256, 256))).float().cuda().unsqueeze(0).unsqueeze(0)
        # self.mask_A = torch.from_numpy(transform.resize(self.mask_A_mid, (512, 512))).float().cuda().unsqueeze(0).unsqueeze(0)
        ...
        # print((self.mask_B).shape)
        '''
        # print((self.real_A != self.real_A).any())
        # print((self.real_B != self.real_B).any())
        print((self.fake_A != self.fake_A).any())
        if (self.fake_A != self.fake_A).any():
            print("nan")
            print((self.image_pathsB))
        #print((self.real_B != self.real_B).any())
        #print((self.image_pathsA))
        print((self.image_pathsB))
        # print((self.fake_B != self.fake_B).any())
        # print((self.mask_A != self.mask_A).any())
        # print((self.mask_B != self.mask_B).any())
        print("stop")
        '''
